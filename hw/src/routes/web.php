...
Route::get('/', function () {
    return view('index' . config('sys.theme.index_theme',0));
});
...
Route::prefix('home')->middleware(['auth', 'auth.session:web'])->namespace('Home')->group(function () {
    Route::get('/', function () {
        return view('home.' . config('sys.theme.home_theme',0) . '.index');
    });
    Route::get('srv', function () {
        return view('home.' . config('sys.theme.home_theme',0) . '.srv');
    });
    Route::get('point', function () {
        return view('home.' . config('sys.theme.home_theme',0) . '.point');
    });
    Route::get('profile', function () {
        return view('home.' . config('sys.theme.home_theme',0) . '.profile');
    });
    Route::get('hd', function () {
        return view('home.' . config('sys.theme.home_theme',0) . '.dh');
    });
...
Route::prefix('admin')->middleware('auth:admin', 'auth.session:admin')->namespace('Admin')->group(function () {
    ...
    Route::prefix('jf')->group(function () {
        Route::post('/', 'jfController@post');
        Route::get('zj', function () {
            return view('admin.jf.zj');
        });
    });
});
